...
export function EmptyState({ agent, userName, onSend, loading }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Online badge */}
      <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-medium mb-6 ${
        agent === "kronos" ? "bg-kronos/10 text-kronos" : "bg-kira/10 text-kira"
      }`}>
        <span className={`w-1.5 h-1.5 rounded-full ${agent === "kronos" ? "bg-kronos" : "bg-kira"}`} />
        En línea
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-sm w-full">
        {suggestions.map((s, i) => (
          <motion.button
            ...
            onClick={() => onSend(s)}
            disabled={loading}
            className="px-4 py-2.5 rounded-xl border border-border-subtle text-xs text-text-muted hover:bg-bg-surface hover:text-text hover:border-border-subtle/80 transition-all disabled:opacity-50 active:scale-95 text-left"
          >
            {s}
          </motion.button>
        ))}
